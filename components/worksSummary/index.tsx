import React, { useRef } from 'react';
import scss from '@/components/worksSummary/main.module.scss';

export default function WorksSummary(): JSX.Element {
  const cinemagicRef: React.RefObject<HTMLElement> = useRef(null);
  const reshareRef: React.RefObject<HTMLElement> = useRef(null);
  const cleantableRef: React.RefObject<HTMLElement> = useRef(null);

  const handleCinemagicScrollDownClick = (): void => {
    cinemagicRef.current!.scrollIntoView({
      behavior: "smooth"
    });
  }

  const handleReshareScrollDownClick = (): void => {
    reshareRef.current!.scrollIntoView({
      behavior: "smooth"
    });
  }

  const handleCleanTableScrollDownClick = (): void => {
    cleantableRef.current!.scrollIntoView({
      behavior: "smooth"
    });
  }

  return (
    <>
      <div 
        className={scss.worksSummaryWrapper} 
        data-scroll-section
      >
        <div className={scss.contentWrapper}>
          <div className={scss.worksSummaryWrapper_summary}>
            <div 
              className={scss.worksSummaryWrapper_left} 
              data-scroll
            >
              <h2 id={scss.worksSummary_title}>
                Works
              </h2>
            </div>
            <div 
              className={scss.worksSummaryWrapper_right} 
              data-scroll
            >
              <ul className={scss.works_summary_list}>
                <li className={scss.works_summary_list_item}>
                  <a href="#cinemagic" data-scroll-to>01. Cinemagic</a>
                </li>
                <li className={scss.works_summary_list_item}>
                  <a href="#reshare" data-scroll-to>02. Reshare</a>
                </li>
                <li className={scss.works_summary_list_item}>
                  <a href="#cleantable" data-scroll-to>03. Clean Table</a>
                </li>
              </ul>
            </div>
          </div>
        </div>
      </div>

      <div 
        className={scss.cinemagicWrapper}
        data-scroll-section  
      >
        <div className={scss.contentWrapper}>
          <div className={scss.leftWrapper}>
            <div>
              
            </div>
          </div>
          <div className={scss.rightWrapper}>

          </div>
        </div>
      </div>
    </>
  )
}